...
export default function StatsScreen({ onBack }) {
  ...
  const renderStatCard = (diff, stats, source) => {
    if (!stats || !stats[diff]) {
      return (
        <div key={diff} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4">
          <div className={`font-bold ${DIFFICULTY_COLORS[diff]}`}>{DIFFICULTY_LABELS[diff]}</div>
          <div className="text-center py-2 text-sm text-neutral-500">暂无记录</div>
        </div>
      );
    }
    const s = stats[diff];
    ...
    return (
      <div key={`${diff}-${source}`} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <div>
            ...
            <div className="text-xs text-neutral-500">胜/总</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-xs text-neutral-500">连胜/最佳</div>
          </div>
        </div>
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-4">
        ...
        {user && (
          <div className="space-y-2">
            <h2 className="text-sm font-semibold text-neutral-500 px-1">云端统计</h2>
            {cloudLoading ? (
              <div className="text-center py-4 text-neutral-500">加载中...</div>
            ) : cloudStats ? (
              DIFFICULTIES.map(d => renderStatCard(d, cloudStats, 'cloud'))
            ) : (
              <div className="text-center py-4 text-neutral-500">暂无云端数据</div>
            )}
          </div>
        )}
        ...
        {/* 本地统计 */}
        <div className="space-y-2">
          <h2 className="text-sm font-semibold text-neutral-500 px-1">本地统计</h2>
